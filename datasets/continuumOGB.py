...
class ContinuumOGB(VisionDataset):
    def __init__(self, root='~/.dgl', name='cora', data_type='train', download=True, task_type=0):
        ...
        ## add self loop
        self_loop = torch.LongTensor(list(range(self.num_nodes)))
        ...
        elif data_type in ['train','test','valid']: # class incremental; use test and train as train
            self.mask = torch.LongTensor(self.idx_split[data_type]) # use val as test, since val is larger than test
        else:
